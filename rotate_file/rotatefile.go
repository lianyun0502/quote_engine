...
func New(p string, options ...Option) (*RotateFile, error) {
	globPattern := p
...
func (rl *RotateFile) genFilename() string {
	now := rl.clock.Now()
...
func (rl *RotateFile) Write(p []byte) (n int, err error) {
...
func (rl *RotateFile) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
...
func (rl *RotateFile) CurrentFileName() string {
	rl.mutex.RLock()
...
// SIGHUP
func (rl *RotateFile) Rotate() error {
	rl.mutex.Lock()
	defer rl.mutex.Unlock()
...
func (rl *RotateFile) rotate_nolock(filename string) error {
...
// the object.
func (rl *RotateFile) Close() error {
	rl.mutex.Lock()
	defer rl.mutex.Unlock()
